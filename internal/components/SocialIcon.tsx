// Inline SVG paths ported from the previous Starlight framework's SocialIcons.astro - keyed
// by label since docs.config.ts's fixed social links (GitHub/Notion/TrickFire Robotics, see
// framework/config/resolve.ts) don't carry a per-icon identifier beyond "github" | "external".
...
export function SocialIcon({ label }: { label: string }) {
    return (
        <svg
            ...
            fill="currentColor"
            className="size-4"
            dangerouslySetInnerHTML={{ __html: ICON_PATHS[label] ?? ICON_PATHS.GitHub }}
        />
    );
}
